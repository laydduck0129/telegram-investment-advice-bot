...
def process(df, min_max_scaler, model):
    ...
    probabilities = [pred[signal] for pred, signal in zip(predictions, signals)]
    # probabilities = [round(pred[signal], 2) for pred, signal in zip(predictions, signals)]
    ...
    return signals[-1], probabilities[-1]
